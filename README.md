...
<sub>真实工程图纸（**6 根轴 / 119 个流程**）：左侧是对象树（顶部带**快速筛选**框，格式为 `名称（S:起点,E:终点,D:时长）`），
中间是画布（横向为时间、轴名在左侧），右侧是属性栏，红色虚线是流程之间的**链接**与投影关系。
这是 0.5 倍的全貌。</sub>

![细节](样张/细节.png)

<sub>放大到原始分辨率：每个流程标注 `S:起点, E:终点, 时长`，轴上是刻度与单位。
缩放有两种语义 —— **保真**（缩放只改变时间轴疏密、字号保持可读，适合对时间、看全局）与
**等比**（整张图按比例放大，适合看细节）。</sub>
